...
#+begin_src lisp
  ...
  (defmethod handle-sdl2-window-event ((key (eql :exposed)) sheet stamp d1 d2)
    ...
    ;; The call to SDL-GET-WINDOW-SURFACE is for side the effect to ensure that
    ;; the surface is allocated (to be able to call SDL-UPDATE-WINDOW-SURFACE).
...
#+begin_example
MCCLIM-SDL2> (open-plain-sheet :sdl2)

 <INFO> [16:52:41] mcclim-sdl2 mirror.lisp (handle-sdl2-event :clim-create-window) -
  Creating a new window.
#<PLAIN-SHEET {100B692353}>
 <INFO> [16:52:41] mcclim-sdl2 mirror.lisp (handle-sdl2-window-event :exposed) -
  Exposing a window.
 <INFO> [16:52:41] mcclim-sdl2 mirror.lisp (handle-sdl2-window-event :exposed) -
  Exposing a window.
 <INFO> [16:53:00] mcclim-sdl2 mirror.lisp (handle-sdl2-window-event :close) -
  Destroying a window.
 <INFO> [16:53:00] mcclim-sdl2 plain-sheet.lisp (handle-event plain-sheet) -
  Unhandled event CLIM:WINDOW-MANAGER-DELETE-EVENT has arrived.
 <INFO> [16:53:00] mcclim-sdl2 port.lisp (handle-sdl2-event :quit) -
  Quit requested... ignoring
MCCLIM-SDL2> (close-plain-sheet *)
NIL
 <INFO> [16:53:11] mcclim-sdl2 mirror.lisp (handle-sdl2-event :clim-destroy-window) -
  Destroying window 2.
#+end_example

Note that when the handler returns a clim event then this event is distributed
to the target sheet; otherwise nothing happens. This is why only unhandled
events of type ~window-manager-delete-event~ are seen. Enable the log level
~:debug~ to see unprocessed sdl2 window events.
>>>>>>> 00a5ee54 (backends: sdl2: implement necessary window (mirror) managament)
