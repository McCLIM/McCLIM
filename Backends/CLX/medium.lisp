  ...
  (let ((ellipse (make-ellipse* center-x center-y
    ...
    (with-clx-graphics () medium
      (multiple-value-bind (x1 y1 width height)
        ...
        (labels ((ellipse-border-p (x-orig y-orig)
                   (let ((tr (climi::polar->screen ellipse))
                         (start-angle (ellipse-start-angle ellipse))
                         (end-angle (ellipse-end-angle ellipse)))
                     (multiple-value-bind (x y)
                         (untransform-position tr x-orig y-orig)
                       (and (<= (- 1.0 .05) (+ (* x x) (* y y)) (+ 1.0 .05))
                            (or (null start-angle)
                                (climi::arc-contains-angle-p
                                 (climi::%ellipse-position->angle
                                  center-x center-y x-orig y-orig)
                 ...
                 (maybe-draw-border-points (line)
                   (multiple-value-bind (lx1 ly1) (line-start-point* line)
                     (when (ellipse-border-p lx1 ly1) (draw-point lx1 ly1)))
                   (multiple-value-bind (lx2 ly2) (line-end-point* line)
                     (when (ellipse-border-p lx2 ly2) (draw-point lx2 ly2))))
                 (draw-line-1 (line)
                   (multiple-value-bind (lx1 ly1) (line-start-point* line)
                     (multiple-value-bind (lx2 ly2) (line-end-point* line)
