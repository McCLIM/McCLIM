...
(defmethod medium-gcontext :before ((medium clx-medium) ink)
  (unless (slot-value medium 'gc)
    (let* ((port (port medium))
           (mirror (port-lookup-mirror port (medium-sheet medium)))
           (gc (xlib:create-gcontext :drawable mirror)))
      (setf (slot-value medium 'gc) gc
            (xlib:gcontext-fill-style gc) :solid))))

(defmethod medium-gcontext ((medium clx-medium) (ink color))
  (declare (optimize (debug 3)))
  (let* ((port (port medium))
         (gc (slot-value medium 'gc))
         (last-medium-device-region (slot-value medium 'last-medium-device-region)))
    ...
    #+no (let ((fn (text-style-mapping port (medium-text-style medium))))
    ...
    (let ((device-region (medium-device-region medium)))
      (unless (eq last-medium-device-region device-region)
        (setf (slot-value medium 'last-medium-device-region) device-region)
