      ...
      (when q
        (if (cdr q)
            ...
            (loop
              with current = q
              while (cdr current)
              do (setq current (cdr current))
              finally (let ((event (cdr current)))
...
(defmethod process-next-event ((port gtk-port) &key wait-function (timeout nil))
  ...
  (let ((event (read-event-from-queue-or-wait port :timeout timeout)))
    (if event
        (progn
          (log:info "Distributing event: ~s" event)
...
(defun process-configure-event (port widget event sheet)
  ...
  (log:info "window size: (~s,~s)" (gdk:gdk-event-configure-width event) (gdk:gdk-event-configure-height event))
