...
(defclass clxv2-medium (clx-medium)
  ())

(defmethod clim-clx::medium-gcontext ((medium clxv2-medium) (ink climi::indexed-pattern))
...
(defmethod clim-clx::medium-gcontext ((medium clxv2-medium) (ink climi::rectangular-tile))
  (multiple-value-bind (mx my)
      ;; For unmirrored sheet we need to apply the native transformation.
      ;; May be it is the wrong place to do it.
      (transform-position (sheet-native-transformation (medium-sheet medium)) 0 0)
...
(defmethod clim-clx::medium-gcontext ((medium clxv2-medium) (ink climi::transformed-design))
...
(defmethod medium-draw-rectangle-using-ink* ((medium clxv2-medium) (ink t) left top right bottom filled)
