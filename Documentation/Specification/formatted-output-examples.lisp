...
(defun table-test (count)
  (with-open-file (file-stream #P"table-example.pdf"
                               :element-type '(unsigned-byte 8)
                               :direction :output
                               :if-does-not-exist :create
                               :if-exists :supersede)
    (clim-pdf:with-output-to-pdf-stream (stream file-stream :scale-to-fit t)
      (clim:with-drawing-options (stream :ink +red+ :text-size 20)
        (fresh-line stream)
...
(defun graph-test (&key (orientation :horizontal))
  (with-open-file (file-stream #P"graph-example.pdf"
                               :element-type '(unsigned-byte 8)
                               :direction :output
                               :if-does-not-exist :create
                               :if-exists :supersede)
...
(defun border-test ()
  (with-open-file (file-stream #P"border-example.pdf"
                               :element-type '(unsigned-byte 8)
                               :direction :output
                               :if-does-not-exist :create
                               :if-exists :supersede)
    (clim-pdf:with-output-to-pdf-stream (stream file-stream)
      (fresh-line stream)
