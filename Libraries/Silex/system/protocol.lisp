...
(define-protocol "Appendices" ())

;;; Fragile
(define-protocol "Utilities" ("Appendices")
  (:function* get-environment-variable (string))
  (:macro* with-system-redefinition-allowed (&body body))
  (:type* function-designator)
  (:function* lerp (v a b))
  (:function* unlerp (v a b))
  (:macro* letf ((&rest forms) &body body &environment env))
  (:function* map-repeated-sequence (result-type n function sequence))
  (:macro* do-sequence ((vars sequence &optional result-form) &body body))
  ...
  (:macro* orf (&rest args) or)
  (:macro* clampf (min max) clamp)
  ...
  (:function* maybe-funcall (function &rest args))
  (:function* maybe-apply (function &rest args))
  (:function* remove-keywords (arg-list keywords))
  (:macro* with-keywords-removed ((var keywords &optional (new-var var)) &body body))
  (:function* symbol-concat (&rest symbols))
  (:function* gen-invoke-trampoline (fun to-bind to-pass body))
  (:macro* with-current-source-form ((form &rest more-forms) &body body))
  (:function* delete-1 (item list &key (test #'eql) (key #'identity)))
  (:function* copy-sequence-into-vector (sequence))
  (:function* rebind-arguments (arg-list))
  (:function* shallow-copy-object (original &optional (new-class (class-of original))))
  (:macro* dolines ((line string &optional result) &body body))
...
(define-protocol "B The CLIM-SYS Package" ("Appendices")
  ("B.1 Resources"
   (:macro defresource (name parameters &key constructor initializer deinitializer matcher initial-copies))
   (:macro using-resource ((variable name &rest parameters) &body body))
   (:function allocate-resource (name &rest parameters))
   (:function deallocate-resource (name object))
   (:function clear-resource (name))
   (:function name-resource (fun name)))
  ("B.2 Multiprocessing"
   (:variable *multiprocessing-p*)
   (:function make-process (fun &key name))
   (:function destroy-process (process))
   (:function current-process ())
   (:function all-processes ())
   (:function processp (object))
   (:function process-name (process))
   (:function process-state (process))
   (:function process-whostate (process))
   (:function process-wait (reason predicate))
   (:function process-wait-with-timeout (reason timeout predicate))
   (:function process-yield ())
   (:function process-interrupt (process fun))
   ...
   (:function atomic-incf (reference))
   (:function atomic-decf (reference)))
  ("B.3 Locks"
   (:function make-lock (&optional name))
   (:macro with-lock-held ((place &optional state) &body body))
   (:function make-recursive-lock (&optional name))
   (:macro with-recursive-lock-held ((place &optional state) &body body)))
  ("B.4 Multiple Value setf"
   (:macro defgeneric* (name lambda-list &body options))
   (:macro defmethod* (name lambda-list &body body))))
