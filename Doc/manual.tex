...
A CLIM sheet in an abstract concept with an infinite \gloss{drawing
...
sheet coordinates to native coordinates, so that drawing can occur on
the (not necessarily immediate) mirror of the sheet.  It would
therefore be enough for sheets that support the \gloss{output
...
the computation of the native transformation. 
Thus, in McCLIM, even sheets that are mute for output have a native
transformation. 

In McCIM, every grafted sheet also has a \gloss{native region}.  The
...
manager.  It may be the \gloss{identity transformation} or a
transformation from millimeters to pixels, or whatever the frame
manager decides. 

\item The native transformation of any mirrored sheet other than the
graft is the composition of the inverse of its mirror transformation,
...
Of course, the specification is unclear hear.  Panes are subclasses of
...
would be the case only of the native transformation of the graft is
...
millimeters as opposed to pixels.  The native transformation of the
graft of such a frame manager is a scaling with coefficients other
than 1. 
...
parallel (called the \gloss{mirror}) in the host windowing system.  A
mirrored sheet is always an instance of the class
...
only if it currently has a mirror.  There may be several reasons for
an instance of that class not to currently have a mirror.  One is that
the sheet is not \gloss{grafted}.  Only grafted sheets can have
mirrors.  Another one is that the \gloss{frame manager} responsible
for the look and feel of the sheet hierarchy may decide that it is
inappropriate for the sheet to have a mirror, for instance if the
underlying windowing system does not allow nested windows inside an
application, or that it would simply be a better use of resources not
to create a mirror for the sheet.  An example of the last example
would be a stream pane inside a the \gloss{viewport} of a
\gloss{scroller pane}.  The graphics objects (usually text) that
appear in a stream pane can have very large coordinate values, simply
because there are many lines of text.  Should the stream pane be
mirrored, the coordinate values used on the mirror may easily go
beyond what the underlying windowing system accepts.  X11, for
instance, can not handle coordinates greater than 64k (16 bit unsigned
integer).  By not having a mirror for the stream pane, the coordinates
will be translated to those of the mirror of the \gloss{viewport}
...
system of its mirror (direct or not).  Thus, a sheet without a mirror
