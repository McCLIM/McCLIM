...
To get an idea of the complexity involved, let us imagine a
\gloss{mirrored sheet} $s$, which has a nontrivial native
transformation, for instance one that is used to draw PostScript
commands.  The coordinate system of such a sheet has an $x$ axis that
grows to the right, and a $y$ axis that grows upwards.  Let us further
imagine that the region of $s$ is some kind of oval shape as shown in
figure \ref{fignative}.
...
\item The native transformation of any mirrored sheet other than
