...
(defgeneric record-line-vertical-offset (pane view line-number)
  (:documentation "Record the end of the line `line-number' (>=
1) as whereever the text-cursor is for `pane' in the view object
`view'."))

(defmethod record-line-vertical-offset ((pane clim-stream-pane) (view drei-buffer-view) (line-number integer))
...
(defgeneric line-vertical-offset (pane view line-number)
  (:documentation "Return the horizontal position (in device
units) of the line with the given number in `view' on
`pane'. `Pane' is used to get text style information and so
forth. `Line-number' should be >= 1."))

(defmethod line-vertical-offset ((pane clim-stream-pane) (view drei-buffer-view)
                                 (line-number integer))
...
(defmethod stream-add-output-record :after ((stream extended-output-stream)
                                            (drei drei-area))
  #+nil(dolist (cursor (cursors drei))
...
(defmethod offset-to-screen-position :around ((pane clim-stream-pane) (view drei-syntax-view) offset)
  "Adjust the returned offset with the position of the Drei area
on display."
  (multiple-value-bind (x y height style-width) (call-next-method)
    (values (+ x (first (aref (cursor-positions view) 0))) y height style-width)))

(defun display-drei-area (drei)
  (with-accessors ((stream editor-pane)
                   (view view)) drei
    #+nil(update-syntax-for-display (buffer drei) (syntax (buffer drei)) (top drei) (bot drei)) ;; FIXME?
    (replay drei stream)
