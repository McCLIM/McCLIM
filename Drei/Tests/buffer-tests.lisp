...
that they are initialized properly."
  (let* ((buffer (make-instance %%buffer))
    ...
    (is (= (offset low) 0))
    (is (= (offset high) 0))
    (is (null (modified-p buffer)))
    (is (eq (buffer low) (buffer high)))
...
order, and that the buffer is updated appropriately."
  (let ((buffer (make-instance %%buffer)))
    ...
    (is (= (offset (low-mark buffer)) 0))
    ;; The upper bound of the change.
    (is (= (offset (high-mark buffer)) 1))
    ;; It should have recorded the modification.
    (is-true (modified-p buffer))
    ;; The buffer should have increased in size the accomodate the
    ;; object.
    (is (= (size buffer) 1))
...
works."
  (let ((buffer (make-instance %%buffer)))
    ...
    (is (= (offset (low-mark buffer)) 0))
    (is (= (offset (high-mark buffer)) 0))
    (is-true (modified-p buffer))
    (is (= (size buffer) 0))
    (insert-buffer-sequence buffer 0 "climacs")
    (delete-buffer-range buffer 0 3)
    (is (= (offset (low-mark buffer)) 0))
    (is (= (offset (high-mark buffer)) 4))
    (is-true (modified-p buffer))
    (is (= (size buffer) 4))
    (is (string= (buffer-substring buffer 0 4) "macs"))
    (insert-buffer-sequence buffer 0 "climacs")
    (delete-buffer-range buffer 3 4)
    (is (= (size buffer) 7))
    (is (string= (buffer-substring buffer 0 3) "cli"))
...
(buffer-test insert-object
  (let ((buffer (make-instance %%buffer)))
    ...
    (let ((m (clone-mark (low-mark buffer) :left)))
      ...
      (is (= (size buffer) 8))
      (is (= (offset m) 3))
      (is (string= (buffer-substring buffer 0 8) "cliXmacs")))
    (let ((m (clone-mark (low-mark buffer) :right)))
      (setf (offset m) 2)
      (insert-object m #\X)
      (is (= (size buffer) 9))
      (is (= (offset m) 3))
      (is (string= (buffer-substring buffer 0 9) "clXiXmacs")))))
...
(buffer-test delete-region
  ...
  (let ((buffer (make-instance %%buffer)))
    ...
    (let ((m (clone-mark (low-mark buffer) :left))
      ...
      (is (= (offset m) 1))
      (is (= (offset m2) 1))
      (is (string= (buffer-substring buffer 0 3) "ccs")))))
...
(buffer-test object-before
  (let ((buffer (make-instance %%buffer)))
    (insert-buffer-sequence buffer 0 "climacs")
    (is (char= (object-before (high-mark buffer)) #\s)))
  (handler-case
      (let ((buffer (make-instance %%buffer)))
	(insert-buffer-sequence buffer 0 "climacs")
...
(buffer-test object-after
  (let ((buffer (make-instance %%buffer)))
    (insert-buffer-sequence buffer 0 "climacs")
    (is (char= (object-after (low-mark buffer)) #\c)))
  (handler-case
      (let ((buffer (make-instance %%buffer)))
	(insert-buffer-sequence buffer 0 "climacs")
...
(buffer-test region-to-sequence
  (let ((seq "climacs")
    ...
    (let ((seq2 (region-to-string (low-mark buffer) (high-mark buffer))))
      (is-true (not (eq seq seq2)))
      (is (string= seq2 "climacs"))))
  (let ((seq "climacs")
	(buffer (make-instance %%buffer)))
    (insert-buffer-sequence buffer 0 seq)
    (let ((seq2 (region-to-string 0 (high-mark buffer))))
      (is-true (not (eq seq seq2)))
      (is (string= seq2 "climacs"))))
  (let ((seq "climacs")
	(buffer (make-instance %%buffer)))
    (insert-buffer-sequence buffer 0 seq)
    (let ((seq2 (region-to-string (high-mark buffer) 0)))
      (is-true (not (eq seq seq2)))
      (is (string= seq2 "climacs"))))
  (let ((seq "climacs")
	(buffer (make-instance %%buffer)))
    (insert-buffer-sequence buffer 0 seq)
    (let ((seq2 (region-to-string (low-mark buffer) 7)))
      (is-true (not (eq seq seq2)))
      (is (string= seq2 "climacs"))))
  (let ((seq "climacs")
	(buffer (make-instance %%buffer)))
    (insert-buffer-sequence buffer 0 seq)
    (let ((seq2 (region-to-string 7 (low-mark buffer))))
      (is-true (not (eq seq seq2)))
      (is (string= seq2 "climacs"))))
  (let ((seq "climacs")
	(buffer (make-instance %%buffer)))
    (insert-buffer-sequence buffer 0 seq)
    (is (string= (region-to-string (high-mark buffer) (low-mark buffer))
                 "climacs")))
  (signals error
    (let ((buffer1 (make-instance %%buffer))
          (buffer2 (make-instance %%buffer)))
      (region-to-string (low-mark buffer1) (high-mark buffer2)))))
