...
(defun aa-render-draw-fn (pixels clipping-region design)
  (lambda (x y alpha)
    (declare (type fixnum x y alpha))
    (setf alpha (min (abs alpha) 255))
    (unless (or (zerop alpha)
                (and clipping-region
                     (not (region-contains-position-p clipping-region x y))))
...
(defun aa-render-xor-draw-fn (pixels clipping-region design)
  (lambda (x y alpha)
    (declare (type fixnum x y alpha))
    (setf alpha (min (abs alpha) 255))
    (unless (or (zerop alpha)
...
#+ (or)
(defun aa-render-alpha-draw-fn (pixels clipping-region)
  (lambda (x y alpha)
    (declare (type fixnum x y alpha))
    (setf alpha (min (abs alpha) 255))
    (unless (or (zerop alpha)
