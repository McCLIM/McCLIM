  ...
  (let ((pixels (climi::pattern-array image)))
    (lambda (x y alpha)
      ...
      (unless (or (zerop alpha)
        ...
        (let* ((value (climi::%rgba-value (climi::design-ink design x y)))
      ...
      (unless (or (zerop alpha)
        ...
        (multiple-value-bind (r.fg g.fg b.fg a.fg)
            (%rgba->vals (let* ((ink (climi::design-ink design x y))
                                (d1 (climi::design-ink (slot-value ink 'climi::design1) x y))
                                (d2 (climi::design-ink (slot-value ink 'climi::design2) x y)))
