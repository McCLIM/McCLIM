...
;;; Mirror
;;;
(defclass rgb-image-mirror-mixin (image-mirror-mixin)
  ())
...
;;; saving
(defmethod save-mirror-image-to-file ((mirror rgb-image-mirror-mixin) file format)
  (declare (ignore format))
  (error "Cannot write image file: ~S" file))
    
(defmethod save-mirror-image-to-stream ((mirror rgb-image-mirror-mixin) stream format)
  (declare (ignore format))
  (error "Cannot write image stream: ~S" stream))

;;; for port
(defmethod %create-mirror-image ((mirror rgb-image-mirror-mixin) width height)
...
;;; render 
;;;
(defmethod %make-image-mirror-get-function ((mirror rgb-image-mirror-mixin))
...
(defmethod %make-image-mirror-set-function ((mirror rgb-image-mirror-mixin))
  (let ((data (climi::image-data (image-mirror-image mirror))))
