...
(define-application-frame demodemo ()
  ...
  (:layouts
   (default
    (spacing (:thickness 8)
     (vertically (:equalize-width t :y-spacing 8)
       (labelling (:label "McCLIM Demos"
                   :text-style (make-text-style :sans-serif :roman :huge)
                   :align-x :center))
       (climi::bordering ()
         (labelling (:label (format nil "Each button starts a demo or ~
                                        test. Multiple demos or tests ~
                                        can run at the same time.")
                     :background +beige+)))
       (horizontally ()
         (vertically ()
           (labelling (:label "Demos")
             (vertically (:equalize-width t :y-spacing 4)
